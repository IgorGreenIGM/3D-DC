...
void test_if_build_method_works_to_get_all_datas(const std::string &file_path, int mat_size, int queue_size, int buffer_size, double del1 = 1, double del2 = 1)
{
    ...
    const auto &all = queue.get_linear(false, false); std::cout << "get linear all datas okk--\n";
...
void test_if_filter_then_unfilter_works(const std::string &file_path, int mat_size, int queue_size, int buffer_size, double del1 = 1, double del2 = 1)
{
    ...
    auto fltrs(queue.filter(true, true));
    std::cout << "filter queue okk--\n";

    std::cout << "matrix number inside the queue : " << build_nb << "--\n";
    std::cout << "filter size : " << fltrs.size() << "\n";
    queue.unfilter(fltrs, true, true);
    std::cout << "unfilter queue okk--\n";

    const auto &datas = queue.get_linear(false, false);
...
void test_if_unfilter_works_to_read_and_unfilters_datas(const std::string &file_path, int mat_size, int queue_size, int buffer_size, double del1 = 1, double del2 = 1)
{
    ...
    const auto &all_datas = queue.get_linear(true, false);
...
{
    // argv[1] = file path + name, argv[2] = matrix size, argv[3] = queue size

    struct stat st;
    stat(argv[1], &st);
    // test if build method works fine, the we'll try with the whole file, creating a buffer of same size of the file
    // test_if_build_method_works_to_get_all_datas(argv[1], std::stoi(argv[2]), std::stoi(argv[3]), st.st_size, 2, 2);

    // test if filter method works fine (tested 3D filters id postions, filters dict size)
    // test_if_filter_works_to_generate_filter_dictionnary(argv[1], std::stoi(argv[2]), std::stoi(argv[3]), st.st_size, 2, 2);

    // test if filter the unfilter works fine
    test_if_filter_then_unfilter_works(argv[1], std::stoi(argv[2]), std::stoi(argv[3]), st.st_size, 1, 1);

    // test if unfilter method works fine (unfiltering and comparate to original)
    // test_if_unfilter_works_to_read_and_unfilters_datas(argv[1], std::stoi(argv[2]), std::stoi(argv[3]), st.st_size, 2, 2);
